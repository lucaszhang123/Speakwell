...
export function scoreSpeaking(audio, transcript) {
...
export async function createAudioTracker(stream) {
  ...
  return {async stop() { clearInterval(timer); source.disconnect(); await context.close(); return summarizeAudio({levels, pitches, totalFrames, voicedFrames}); }};
}
